...
PosixSocket::IsConnected()
{
    ...
    {
        this->SetToLastSocketError();
        n_printf("PosixSocket::IsConnected(): select() failed with '%s'!\n", this->GetErrorString().AsCharPtr());
        return false;
    }
    else if (0 == res)
    {
...
PosixSocket::Send(const void* buf, SizeT numBytes, SizeT& bytesSent)
{
    ...
    if (SOCKET_ERROR == res)
    {
        ...
        {
            this->SetToLastSocketError();
            n_printf("PosixSocket::Send(): send() failed with '%s'\n", this->GetErrorString().AsCharPtr());
            return Error;
        }
    }
    bytesSent = res;
...
PosixSocket::HasRecvData()
{
    ...
    {
        this->SetToLastSocketError();
        n_printf("PosixSocket::HasRecvData(): select() failed with '%s'!\n", this->GetErrorString().AsCharPtr());
        return false;
    }
    else if (0 == res)
    {
...
PosixSocket::Recv(void* buf, SizeT bufSize, SizeT& bytesReceived)
{
    ...
    else if (SOCKET_ERROR == res)
    {
        ...
        this->SetToLastSocketError();
        n_printf("PosixSocket::Recv(): recv() failed with '%s'\n", this->GetErrorString().AsCharPtr());
        return Error;
    }
    else
    {
...
PosixSocket::SocketErrorToErrorCode(int wsaErrorCode)
{
    switch (wsaErrorCode)
    {
        ...
        case EPROCLIM:           return ErrorTooManyProcesses; break;       
        default:
            return ErrorUnknown;
            break;
    }
