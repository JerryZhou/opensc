...
GLboolean StringIsInExtensionString(const char* string, const GLubyte* extensions)
{
    const GLubyte* start;
    GLubyte* where;
    GLubyte* terminator;
...
// helper functions to check extensions in both legacy and current methods
GLboolean ExtensionIsSupported(const char* extension)
{
    const GLubyte* extensions;

    // @todo check for valid extension string
...
//    }
    ...
    int majorVersion;
    // @todo determine version from glGetString​(GL_VERSION​) which done support glGetIntegerv​(GL_MAJOR_VERSION​, *);
    glGetIntegerv(GL_VERSION, &majorVersion);
    ...
    if (majorVersion < 3)
    {
        ...
        if (extensions != NULL)
        {
            if (StringIsInExtensionString(extension, extensions))
                return GL_TRUE;
        }
    }
    else
...
int query_NPOT_capability( void )
{
	...
	if( has_NPOT_capability == SOIL_CAPABILITY_UNKNOWN )
	{
		...
		if(ExtensionIsSupported("GL_ARB_texture_non_power_of_two" ))
		{
...
{
	/*	check for the capability	*/
    // @todo: if version is >= 3.1 we have rectagular textures
	if( has_tex_rectangle_capability == SOIL_CAPABILITY_UNKNOWN )
	{
		/*	we haven't yet checked for the capability, do so	*/
		if(ExtensionIsSupported("GL_ARB_texture_rectangle" )
		&&
			ExtensionIsSupported("GL_EXT_texture_rectangle" )
		&&
			ExtensionIsSupported("GL_NV_texture_rectangle" )
			)
		{
			/*	not there, flag the failure	*/
...
int query_cubemap_capability( void )
{
	...
	if( has_cubemap_capability == SOIL_CAPABILITY_UNKNOWN )
	{
		/*	we haven't yet checked for the capability, do so	*/
		if(
			ExtensionIsSupported("GL_ARB_texture_cube_map" )
		&&
			ExtensionIsSupported("GL_EXT_texture_cube_map" )
			)
		{
			/*	not there, flag the failure	*/
...
int query_DXT_capability( void )
{
	...
	{
		/*	we haven't yet checked for the capability, do so	*/
		if( ExtensionIsSupported("GL_EXT_texture_compression_s3tc" ) )
		{
			/*	not there, flag the failure	*/
